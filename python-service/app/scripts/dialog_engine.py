class DialogEngine(object):
    def __init__(self, self.s,logger, topic_names, subscriberID=APP_ID, pkgPath=PKG_PATH):
        #self.language = self.s.ALDialog.getLanguage()
        assert topic_names isinstance(topic_names,list),"topic_names must be list for iterable"
        self.logger = logger
        self.topic_names = topic_names
        self.subscriberID = subscriberID
        self.TOPIC_PATHS = {topic: '{0}/dialog/{1}/{1}_{2}.top'.format(pkgPath, topic, 'mnc') for topic in self.topic_names}
        for topic in self.topic_names:
            self.TOPIC_PATH = self.TOPIC_PATHS[topic]
