...
//v2            var regex = new RegExp("[\\?&]robot=([^&#]*)");
//v2            var results = regex.exec(location.search);
//v2            return results == null ? "" : decodeURIComponent(results[1].replace(/\+/g, " ").replace("/", ""));
            return "nao:nao@192.168.0.193";
        }
        ...
        $.getScript(robotAddress + '/libs/qimessaging/1.0/qimessaging.js', function() {
//v2            QiSession( resolve, reject, qimAddress );
            qis = new QiSession(qimAddress);

              qis.socket().on('connect', function()
              {
                console.log("QiSession connected!");
                if (resolve) resolve(qis);
